...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    */
    ...
    $sql = "SELECT ims_lotto_period.* FROM ims_lotto_period             
            WHERE period_no = :period_no AND period_month = :period_month AND period_year = :period_year ORDER BY lotto_type DESC";
    ...
    if ($stmt->rowCount() > 0) {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            echo "<b> รางวัลที่ได้รับ: " . htmlspecialchars($row['prize']) . "</b>.";
            echo "<b> เลขที่ออก: " . htmlspecialchars($row['lotto_number_result']) . "</b></br>";
